...
public class SuperEnhancedShortMixer extends SimpleMixer{

	protected SuperEnhancedShortMixer(int mixingInterval) {
		...
		if(data.length==2){
			...
			for(int i = 0; i<baseMix.length; i+=2){
				...
				for(int currentTalker : talking){
					baseMix[i] += bytesToShort(data[currentTalker][i+1], data[currentTalker][i]);				//Uses little endian
				}
